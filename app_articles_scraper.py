...
def scrape_24heures(url):
    ...
    pub_date = soup.find(
        "time",
        "FullDateTime_root__K7FL2 ArticleContainer_content-width__FRl7F FullDateTime_-metainfo__e_tHp",
    ).get("datetime")
    datetime_f, readable_f = convert_publication_date(pub_date)
    paragraph_text = soup.find_all(
        "p",
        "ArticleParagraph_root__lhFZo ArticleContainer_content-width__FRl7F link_focus__0ZMwx link_externalicon-big__ZdPgo link_externalicon__qcwXs ArticleElement_article-element__q93eL",
    )
    if paragraph_text != []:
...
def scrape_chiswickcalendar(url):
    ...
    Logging.echo(f"Scrapping...{url}")
    title = soup.find("h1", "post-title entry-title")
    if title is not None:
        title = title.text
    author = soup.find("span", "entry-author-link")
    if author is not None:
        author = author.text
    pub_date = soup.find("time", "date-container minor-meta updated").get("datetime")
    datetime_f, readable_f = convert_publication_date(pub_date)
    sections = []
...
def scrape_corriere(url):
    ...
    soup = BeautifulSoup(page_html, "html.parser")
    Logging.echo(f"Scrapping...{url}")
    title = soup.find("h1", "title-art-hp is-xmedium is-line-h-106 is-mr-b-20")
    if title is not None:
        title = title.text
    author = soup.find("span", "writer is-mr-l-4")
    if author is not None:
        author = author.text
